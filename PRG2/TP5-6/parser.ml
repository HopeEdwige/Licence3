...
(* La liste des mots clés du langage: programmes et annotations *)

let keywords=[",";"(";")";"[";"]";":=";"begin";"end";"while";"do";"done";"if";"then";"else";"skip";";";"+";"-";"*";"=";">";"<";">=";"<=";"->";"!";"&&";"||";"{";"}";"invariant";"endif"]


(* lexer : char Stream.t -> Genlex.token Stream.t 
   Le lexer pour les programmes annotés. 
   On ne traite qu'un cas particulier: le cas des entiers négatifs. Les autres
   cas sont traités par le lexer de base d'Ocaml (make_lexer) *)
...
let rec parse_val = parser
    | [< 'Int n >] -> Const n
    | [< 'Ident f >] -> Var f
    | [< 'Kwd "("; e=parse_expression ?? "Syntaxe d'expression incorrecte"; 'Kwd ")" ?? ("Parenthese manquante à la fin de "^(expr2string e)) >] -> e
...
(* parse_atom: Genlex.token Stream.t -> formule
   ...
   Le parser de littéraux (atomes) et formules *)
...
(* parse_programme: Genlex.token Stream.t -> programme 
   ...
   Le parser de programme et de séquence de programmes
*)
...
and parse_sequence = parser
    [< first= parse_programme ; stream >] -> 
      (parser 
	   [< 'Kwd ";" ; rem=parse_sequence ?? "Mauvaise séquence">] -> first::rem
	 | [< >] -> [first]
      ) stream
  | [< >] -> []
...
(* file_parse: (filename: string) -> progAnnote
   retourne le programme annoté contenu dans le fichier de nom filename
*)
