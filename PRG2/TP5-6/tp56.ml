...
(* Fonction de renommage n�cessaire pour �viter que le r�sultat de la substitution de x par y dans
...
(* ----- Simplifie les expressions arithm�tiques binaires ----- *)
let rec partEval e =
	match e with
		| Const i -> Const i
		| Var v -> Var v
...
(* ----- Simplifie une expression (et en donne le r�sultat) ----- *)
let rec simplif f =
	match f with
		| True -> True
		| False -> False
...
(* ----- Affiche un programme annot� entr� ----- *)
