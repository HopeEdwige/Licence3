		...
		while ((strcmp(command, "exit\n") != 0) && (strcmp(command2, "exit\n") != 0)) {
			...
			// If no error, we can execute the command
				...
				if (result_fork == 0) {
					...
					// Redirect the standard output to the standard input
					if (dup2(0, 1) < 0) {  // Less than 0 if error
						fprintf(stderr, "%s\n", "The io redirection failed.");
						return 1;
					}
					...
					result_execution = execvp(command_parts2[0], command_parts2);

					// If an error occured
					if (result_execution != 0) {
						fprintf(stderr, "%s%d\n", "Error during the execution of the second command. Exit with code ", result_execution);
						return result_execution;
					}
